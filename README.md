...
> **Troubleshooting `404 Not Found` bei Schreibzugriffen (z.B. `Update-ConfluencePage`):** Wenn ein `Get-ConfluencePage` auf dieselbe Seite funktioniert, aber `Update-`/`Remove-ConfluencePage` mit `404` scheitert, liegt das meist nicht an fehlenden Scopes, sondern an **Seiten-Restrictions** (Confluence-Seite → "..." → Restrictions) oder fehlenden Space-Permissions für den Service Account — unabhängig vom OAuth-Scope.
